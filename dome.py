...
class Dome:
    def __init__(self, config_file="dome_config.json"):
        ...
        if self.config.get("testing", {}).get("smoke_test", False):
            ...
            print(f"SMOKE TEST MODE: Using short timeout of {self.MAX_OPEN_TIME}s")
        else:
            self.MAX_OPEN_TIME = (
    ...
    def get_shutter_limits(self):
        ...
        """
        upper_limit = self.dome.analog_in(self.UPPER)  # Return actual analog value
        lower_limit = self.dome.analog_in(self.LOWER)  # Return actual analog value
    ...
    def wait_for_shutter_operation(self, operation_name):
        ...
        """
        print(f"Waiting {self.MAX_OPEN_TIME}s for {operation_name} to complete...")
        elapsed = 0
        while elapsed < self.MAX_OPEN_TIME:
            time.sleep(self.POLL)
            elapsed += self.POLL
            print(f"  {operation_name}... {elapsed:.1f}s elapsed")

        # Stop the movement signal after timeout
        self.shutter_stop()
        print(
            f"{operation_name} operation completed (timed out at {self.MAX_OPEN_TIME}s)"
        )
