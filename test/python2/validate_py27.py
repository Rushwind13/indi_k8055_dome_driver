...
class ValidationTests(object):
    ...
    def test_persistence_functionality(self):
        ...
        try:
            ...
            # Create test config
            config = {
                'pins': {'encoder_a': 1, 'encoder_b': 2, 'home_switch': 3,
                        'shutter_upper_limit': 1, 'shutter_lower_limit': 2,
                        'dome_rotate': 1, 'dome_direction': 2,
                        'shutter_move': 3, 'shutter_direction': 4},
                'calibration': {'home_position': 0.0, 'ticks_to_degrees': 1.0, 'poll_interval': 0.1},
                'hardware': {'mock_mode': True, 'device_port': 0},
                'testing': {'smoke_test': True, 'smoke_test_timeout': 3.0}
            }
            ...
            if dome2.position != 45.0:
                raise Exception("Position not restored correctly: {} != 45.0".format(dome2.position))
            ...
            try:
                ...
                if self.verbose:
                    print("  Connection state not accessible (acceptable for mock mode)")
    ...
    def test_script_integration(self):
        ...
        scripts_dir = os.path.join(script_dir, '..', '..', 'indi_driver', 'python2', 'scripts')
        # Scripts that should have full persistence (restore + save)
        full_persistence_scripts = ['status.py', 'goto.py', 'park.py', 'move_cw.py', 'open.py', 'unpark.py', 'disconnect.py', 'abort.py']
        ...
        try:
            ...
            for script_name in full_persistence_scripts:
                ...
                if 'from persistence import' not in content:
                    raise Exception("Script {} missing persistence import".format(script_name))
                
                if 'restore_state' not in content:
                    raise Exception("Script {} missing restore_state call".format(script_name))
                
                if 'save_state' not in content:
                    raise Exception("Script {} missing save_state call".format(script_name))
            ...
            for script_name in save_only_scripts:
                ...
                self.log("Script {} has save-only persistence integration".format(script_name))
    ...
    def test_python27_compatibility(self):
        ...
        try:
            ...
            if sys.version_info[0] != 2 or sys.version_info[1] != 7:
                print("  ⚠ Warning: Not running on Python 2.7 ({}.{})".format(
