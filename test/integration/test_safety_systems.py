...
class TestSafetySystems(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        try:
            ...
            if getattr(self, "_config_created", False) and os.path.exists(self._config_path):
    ...
    def test_abort_script_availability(self):
        ...
        self.assertTrue(os.access(abort_script, os.X_OK), "Abort script must be executable")
    ...
    def test_safety_timeout_configuration(self):
        ...
        for timeout_key in essential_timeouts:
            self.assertIn(timeout_key, safety_config, f"Missing safety timeout: {timeout_key}")
            timeout_value = safety_config[timeout_key]
            self.assertIsInstance(timeout_value, (int, float), f"Timeout {timeout_key} must be numeric")
            self.assertGreater(timeout_value, 0, f"Timeout {timeout_key} must be positive")

    def test_hardware_mode_safety_validation(self):
        """Test hardware mode safety validation works."""
        if not self.is_hardware_mode:
            self.skipTest("Hardware mode safety validation only applies to hardware mode")
        ...
        self.assertGreaterEqual(operation_timeout, 20, "Hardware mode needs longer safety timeouts")
    ...
    def test_smoke_mode_safety_validation(self):
        ...
        safety_config = config.get("safety", {})
        operation_timeout = safety_config.get("operation_timeout", 0)
        self.assertLessEqual(operation_timeout, 10, "Smoke mode should have shorter safety timeouts")
    ...
    def test_sequential_abort_calls(self):
        ...
        for i in range(3):
            ...
            self.assertEqual(
                result.returncode, 
                0, 
                f"Abort call {i+1} failed: {result.stderr}"
            )
...
def run_safety_tests():
    ...
    if result.wasSuccessful():
        ...
        print()
        print("⚠️  CRITICAL: Do not proceed with hardware testing until safety issues are resolved!")
        return False
