...
def create_test_device(mock_mode=True, device_port=0):
    ...
    try:
        ...
        print("\nCreating K8055 device (mock_mode={}, port={})...".format(mock_mode, device_port))
        
        # Try both interface styles
        try:
            # New k8055 class interface
            device = pyk8055_wrapper.k8055(BoardAddress=device_port, debug=True, mock=mock_mode)
    ...
    for pin in range(1, 6):
        ...
        try:
            ...
            if value >= 0:  # 0 or 1 are valid
                print("  ✓ Pin {} value: {} {}".format(pin, value, "(HIGH)" if value else "(LOW)"))
                results[pin] = True
            else:
...
def test_analog_inputs(device):
    ...
    for channel in range(1, 3):
        ...
        try:
            ...
            if 0 <= value <= 255:
                print("  ✓ Channel {} value: {} ({}%)".format(channel, value, round(value/255.0*100, 1)))
                results[channel] = True
            else:
                print("  ❌ Channel {} ERROR: Value {} out of range (0-255)".format(channel, value))
    ...
    for counter in range(1, 3):
        ...
        try:
            ...
            if reset_result == 0:
                ...
                print("  ⚠️  Counter {} reset returned: {}".format(counter, reset_result))
...
def validate_dome_config(config_file):
    ...
    try:
        ...
        # Check required pins
        required_pins = [
            'encoder_a', 'encoder_b', 'home_switch',
            'shutter_upper_limit', 'shutter_lower_limit',
            'dome_rotate', 'dome_direction',
            'shutter_move', 'shutter_direction'
        ]
            ...
            if pin_name in pins:
                ...
                if pin_name in ['encoder_a', 'encoder_b', 'home_switch']:
                    ...
                    if not (1 <= pin_value <= 5):
                        print("  ⚠️  Digital input pin {} out of range (1-5)".format(pin_value))
                elif 'analog' in pin_name or 'limit' in pin_name:
                    # Analog inputs (1-2)
                    if not (1 <= pin_value <= 2):
                        print("  ⚠️  Analog input channel {} out of range (1-2)".format(pin_value))
                else:
                    # Digital outputs (1-8)
                    if not (1 <= pin_value <= 8):
                        print("  ⚠️  Digital output pin {} out of range (1-8)".format(pin_value))
            else:
        ...
        digital_output_pins = ['dome_rotate', 'dome_direction', 'shutter_move', 'shutter_direction']
...
def interactive_mode():
    ...
    while True:
        ...
        if cmd == 'quit' or cmd == 'q':
            ...
            print("\nCommands available:")
            print("  don <pin>, doff <pin>, din <pin>, ain <ch>, counter <ch>, reset <ch>")
            ...
            try:
                ...
                print("Digital pin {} ON: {}".format(pin, "Success" if result == 0 else "Failed"))
            ...
            try:
                ...
                print("Digital pin {} OFF: {}".format(pin, "Success" if result == 0 else "Failed"))
            except (ValueError, IndexError):
            ...
            try:
                ...
                if hasattr(device, 'ReadDigitalChannel'):
                    ...
                    value = device.digital_in(pin)
                print("Digital input pin {}: {} {}".format(pin, value, "(HIGH)" if value else "(LOW)"))
            except (ValueError, IndexError):
            ...
            try:
                ...
                if hasattr(device, 'ReadAnalogChannel'):
                    ...
                    value = device.analog_in(channel)
                print("Analog input channel {}: {} ({}%)".format(channel, value, round(value/255.0*100, 1)))
            except (ValueError, IndexError):
            ...
            try:
                ...
                print("Counter {} reset: {}".format(counter, "Success" if result == 0 else "Failed"))
            except (ValueError, IndexError):
...
Examples:
  ...
  python k8055_pin_tester.py --help-pins
    ...
    parser.add_argument('--config', '-c', type=str,
                      help='Validate a dome configuration file')
    parser.add_argument('--test-all', '-t', action='store_true',
                      help='Run all pin tests systematically')
    parser.add_argument('--help-pins', '-p', action='store_true',
    ...
    parser.add_argument('--port', type=int, default=0,
                      help='K8055 device port (0-3, default: 0)')
